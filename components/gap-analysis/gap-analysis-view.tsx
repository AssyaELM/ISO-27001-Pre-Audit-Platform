  ...
  return <main className={styles.shell}>
    ...
    <section className={styles.page}>
      <header className={styles.topbar}>
        <div className={styles.breadcrumb}><span>{profile.organization}</span><b>/</b><strong>Gap Analysis</strong></div>
        <div className={styles.account}><span className={styles.language}>EN / FR</span><span className={styles.workspaceState}><i />{fr ? "Espace actif" : "Workspace active"}</span><span className={styles.userName}>{profile.name}</span><ChevronDown size={15} />{profile.avatar ? <Image className={styles.avatarImage} src={profile.avatar} alt="" width={42} height={42} /> : <span className={styles.avatar}>{profile.name.slice(0, 1).toUpperCase() || "N"}</span>}</div>
      </header>

      <section className={styles.board}>
        <header className={styles.boardTitle}><div><h1>Gap Analysis</h1><p>ISO/IEC 27001:2022</p></div></header>
        {error && <p className={styles.error} role="alert">{error}</p>}

        <section className={styles.kpis} aria-label="Gap metrics">
        ...
        <section className={styles.themeSection} aria-label="Gaps by theme">
        ...
        <section className={styles.filters}>
          ...
          <label className={styles.select}><select value={status} onChange={(event) => setStatus(event.target.value as typeof status)} aria-label="Status"><option value="all">{fr ? "Tous les statuts" : "All statuses"}</option><option value="full_gap">Full gap</option><option value="partial_gap">Partial gap</option><option value="clarification_required">Clarification</option><option value="applicability_review_required">Applicability</option></select><ChevronDown size={17} /></label>
          <label className={styles.select}><select value={theme} onChange={(event) => setTheme(event.target.value as typeof theme)} aria-label="Theme"><option value="all">{fr ? "Tous les thèmes" : "All themes"}</option>{themeOrder.map((key) => <option value={key} key={key}>{key}</option>)}</select><ChevronDown size={17} /></label>
          <label className={styles.select}><select value={evidence} onChange={(event) => setEvidence(event.target.value as typeof evidence)} aria-label="Evidence status"><option value="all">Evidence status</option><option value="not_provided">{fr ? "Aucune preuve" : "No evidence"}</option><option value="provided">{fr ? "Fournie" : "Provided"}</option></select><ChevronDown size={17} /></label>
        ...
        <section className={styles.analysisArea}>
          <section className={styles.listPanel} aria-label="Gap list">
          ...
          <section className={styles.detailPanel} aria-live="polite">
            {!selected ? <p className={styles.empty}>{fr ? "Sélectionnez un élément pour afficher son détail." : "Select an item to view its details."}</p> : <>
              ...
              <dl className={styles.details}>
                ...
                {selected.gapCode && <div><dt>Gap code</dt><dd><code>{selected.gapCode}</code></dd></div>}
                ...
                <div><dt>Evidence status</dt><dd><span className={`${styles.evidenceBadge} ${styles[`evidence_${selected.evidenceStatus}`]}`}>{evidenceLabel(selected.evidenceStatus, fr)}</span>{selected.evidenceStatus === "not_provided" && <span className={styles.evidenceNote}>{fr ? "Aucune preuve jointe" : "No evidence attached"}</span>}</dd></div>
              </dl>
