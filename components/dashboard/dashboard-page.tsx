...
export function DashboardPage() {
  ...
  const overdueActionLabel = `${view.remediation.overdue} overdue ${countLabel(view.remediation.overdue, "remediation action")}`;
  const kpis = [
    { label: french ? "Complétude de l'évaluation" : "Assessment completion", value: `${view.completion}%`, detail: `${view.assessedControls} of ${view.totalControls} ${countLabel(view.totalControls, "control")} assessed`, tone: "aqua" },
    ...
    { label: french ? "Gaps ouverts" : "Open gaps", value: String(view.gaps.length), detail: `${view.fullGaps} full · ${view.partialGaps} partial`, tone: "orange" },
    { label: french ? "Couverture Evidence" : "Evidence coverage", value: `${view.evidenceCoverage}%`, detail: `${view.evidenceQuestions} linked ${countLabel(view.evidenceQuestions, "question")}`, tone: "violet" },
    { label: french ? "Remediation terminée" : "Remediation completion", value: `${view.remediation.total ? Math.round((view.remediation.completed / view.remediation.total) * 100) : 0}%`, detail: `${view.remediation.completed} of ${view.remediation.total} completed`, tone: "green" },
    { label: french ? "Préparation documentaire" : "Document readiness", value: `${view.documentReadiness}%`, detail: `${view.documents?.length ?? 0} canonical ${countLabel(view.documents?.length ?? 0, "document")}`, tone: "teal" },
  ];
    ...
    <main className={styles.shell}>
      ...
      <section className={styles.content}>
        <header className={styles.topbar}>
          <button className={styles.menuButton} type="button" onClick={() => setMobileNav((open) => !open)} aria-label="Toggle navigation">Menu</button>
          <div className={styles.crumb}><span>{context.organization}</span><b>/</b><strong>{french ? "Tableau de bord" : "Dashboard"}</strong></div>
          <div className={styles.profileTools}><span className={styles.language}>EN / FR</span><span className={styles.workspaceStatus}><i />{context.workspaceId ? (french ? "Espace actif" : "Workspace active") : (french ? "Configuration" : "Setup")}</span><span className={styles.profileName}>{profileName}</span><span className={styles.avatar}>{initial}</span></div>
        </header>

        <section className={styles.hero} style={{ backgroundImage: `url(${assets.banner})` }}>
          <div><h1>{french ? "Bienvenue" : "Welcome back"}, {profileName}</h1><p>{french ? "Pilotez votre espace de conformite ISO 27001." : "Manage your ISO 27001 compliance workspace."}</p><Link prefetch={true} href="/assessment" className={styles.primary}>{french ? "Continuer l'evaluation" : "Continue assessment"}</Link></div>
        </section>
        ...
        <section className={styles.progressGrid}>
          <article className={`${styles.card} ${styles.donutCard}`}><div className={styles.cardHeading}><div><span className={styles.eyebrow}>{french ? "Progression globale" : "Global progress"}</span><h2>{french ? "Complétude de l'évaluation" : "Assessment completion"}</h2></div><ArrowRight size={17} /></div><div className={styles.donut} style={{ "--progress": `${view.completion * 3.6}deg` } as React.CSSProperties}><div><strong>{view.completion}%</strong><span>{view.assessedControls} / {view.totalControls} {countLabel(view.totalControls, "control")}</span></div></div></article>
          <article className={`${styles.card} ${styles.domainCard}`}><div className={styles.cardHeading}><div><span className={styles.eyebrow}>{french ? "Répartition" : "Breakdown"}</span><h2>Assessment completion by domain</h2></div></div><div className={styles.domainList}>{view.domainReadiness.map((item) => <div className={styles.domainRow} key={item.domain}><span>{item.domain}</span><div><i style={{ width: `${item.value}%` }} /><b>{item.value}%</b></div></div>)}</div><p className={styles.note}>Domain completion reflects answered controls and their recorded assessment outcomes.</p></article>
          <div className={styles.priorityColumn}><article className={`${styles.card} ${styles.gapTypeCard}`}><div className={styles.cardHeading}><h2>{french ? "Gaps ouverts par type" : "Open gaps by type"}</h2></div><div className={styles.gapType}><span><i className={styles.fullDot} />Full gaps</span><strong>{view.fullGaps}</strong><span><i className={styles.partialDot} />Partial gaps</span><strong>{view.partialGaps}</strong></div></article><article className={`${styles.card} ${styles.topControlsCard}`}><div className={styles.cardHeading}><h2>{french ? "Contrôles à traiter" : "Top controls requiring action"}</h2><Link href="/gap-analysis">View all</Link></div>{view.topGaps.map((gap) => <div className={styles.controlRow} key={gap.id}><span><b>{gap.controlCode}</b><small>{gap.controlTitle}</small></span><Link href={gap.href}>View gap</Link></div>)}{!view.topGaps.length && <p className={styles.empty}>{french ? "Aucun gap actif." : "No active gaps."}</p>}</article></div>
        </section>

        <section className={styles.actionGrid}>
          <article className={`${styles.card} ${styles.remediationCard}`}><div className={styles.cardHeading}><div><span className={styles.eyebrow}>Remediation</span><h2>{french ? "Statut des actions" : "Remediation status"}</h2></div><Link href="/remediation-plan">View plan</Link></div><div className={styles.statusGrid}><div><strong>{view.remediation.overdue}</strong><span>Overdue</span></div><div><strong>{view.dueSoon}</strong><span>Due soon</span></div><div><strong>{view.remediation.inProgress}</strong><span>In progress</span></div><div><strong>{view.remediation.completed}</strong><span>Completed</span></div></div></article>
          <article className={`${styles.card} ${styles.evidenceCard}`}><div className={styles.cardHeading}><div><span className={styles.eyebrow}>Evidence Room</span><h2>{french ? "Couverture des preuves" : "Evidence coverage"}</h2></div><Link href="/evidence-room">Open room</Link></div><div className={styles.metricList}><p><span>Questions with evidence</span><b>{view.evidenceQuestions}</b></p><p><span>Questions without evidence</span><b>{Math.max(0, data.responses.length - view.evidenceQuestions)}</b></p><p><span>Linked evidence</span><b>{data.links.length}</b></p><p><span>Unlinked evidence</span><b>{view.unlinkedEvidence}</b></p><p><span>Evidence review overdue</span><b>{view.overdueEvidence}</b></p></div><div className={styles.progressBar}><i style={{ width: `${view.evidenceCoverage}%` }} /></div></article>
          <article className={`${styles.card} ${styles.documentsCard}`}><div className={styles.cardHeading}><div><span className={styles.eyebrow}>AI Documents</span><h2>{french ? "État documentaire" : "Document readiness"}</h2></div><Link href="/ai-documents">Open docs</Link></div><div className={styles.documentStats}>{["missing", "ready", "draft", "finalized"].map((status) => <div key={status}><strong>{data.documents.filter((item) => item.status === status).length}</strong><span>{status}</span></div>)}</div><div className={styles.documentReadiness}><strong>{view.documentReadiness}%</strong><span>Document readiness</span></div></article>
        </section>

        <section className={styles.bottomGrid}>
          <article className={`${styles.card} ${styles.nextSteps}`}><div className={styles.cardHeading}><div><span className={styles.eyebrow}>Action queue</span><h2>{french ? "Prochaines étapes" : "Next steps"}</h2></div></div><StepLink href="/assessment" label={french ? "Compléter les questions restantes" : "Complete remaining assessment questions"} show={view.completion < 100} /><StepLink href="/evidence-room" label={french ? "Ajouter les preuves manquantes" : "Add evidence for unanswered controls"} show={view.evidenceCoverage < 100} /><StepLink href="/remediation-plan" label={`${french ? "Revoir" : "Review"} ${overdueActionLabel}`} show={view.remediation.overdue > 0} /><StepLink href="/ai-documents" label={french ? "Préparer les documents manquants" : "Prepare missing ISO documents"} show={data.documents.some((item) => item.status === "missing")} /><StepLink href="/remediation-plan" label={french ? "Traiter les gaps ouverts" : "Address open gaps"} show={view.gaps.length > 0} />{view.completion === 100 && !view.gaps.length && <p className={styles.empty}>No priority actions detected.</p>}</article>
          <article className={`${styles.card} ${styles.activityCard}`}><div className={styles.cardHeading}><div><span className={styles.eyebrow}>Workspace activity</span><h2>{french ? "Progrès récent" : "Recent progress"}</h2></div></div>{view.recent.map((event, index) => { const Icon = event.icon; return <div className={styles.activityRow} key={`${event.label}-${event.date}-${index}`}><Icon size={17} /><span>{event.label}<small>{formatRelative(event.date, locale)}</small></span></div>; })}{!view.recent.length && <p className={styles.empty}>{french ? "Aucune activité récente." : "No recent activity."}</p>}</article>
        </section>

        <section className={`${styles.card} ${styles.scopeCard}`}><div className={styles.cardHeading}><div><span className={styles.eyebrow}>Workspace context</span><h2>{french ? "Résumé du périmètre SMSI" : "ISMS scope summary"}</h2></div></div><div className={styles.scopeGrid}>{[["Organization", context.organization], ["Company size", context.companySize], ["Country", formatCountry(context.country, locale)], ["Sector", context.industry], ["Scope", context.scope], ["Assessment owner", context.owner || profileName], ["Sites", context.sites], ["Workspace status", context.workspaceId ? "Active" : "Setup"]].map(([label, value]) => <div key={label}><small>{label}</small><strong>{value || "—"}</strong></div>)}</div></section>
