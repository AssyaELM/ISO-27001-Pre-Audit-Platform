...
const typeLabel = (type: string) => ({ policy_process: "Policy / Process", application: "Application", proof_traceability: "Proof / Traceability", conditional: "Conditional" }[type as "policy_process"] ?? type);
...
export default function TechnologicalControlPage() {
  ...
  async function answer(questionId: string, value: AssessmentAnswerValue) {
    ...
    if (!workspaceId || saving || (value === "not_applicable" && !justification)) return;
  ...
  return <main className={styles.shell}>
    ...
    <section className={styles.content}>
      <header className={styles.topbar}><p>{organization} / <strong>Technological Controls</strong></p><span className={styles.language}>EN / FR</span></header>
      <section className={styles.titleCard}><div><h1>Technological Controls</h1><p>ISO/IEC 27001:2022 — Annex A.8</p></div><div className={styles.titleProgress}><span>{control.code} {locale === "fr" ? "sur" : "of"} 34</span><i><b style={{ width: `${completed / 34 * 100}%` }} /></i></div></section>
      <div className={styles.assessmentGrid}>
        <aside className={styles.controls}><div className={styles.controlsHeader}><strong>Technological Controls</strong><small>34 controls</small></div>{technologicalControls.map((candidate) => <a key={candidate.id} href={`/assessment/${window.location.pathname.split("/")[2]}/${candidate.id}`} onClick={(e) => { e.preventDefault(); handleNav(candidate.id); }} className={candidate.id === controlId ? styles.controlActive : ""}><i className={`${styles.status} ${styles[status(candidate.id)] ?? ""}`} /><span><strong>{candidate.code} {candidate.name}</strong><small>{status(candidate.id)}</small></span></a>)}</aside>
        <section className={styles.mainPanel}>
          <header className={styles.controlHeading}><div><span>Technological control</span><h2>{control.code} <b>{control.name}</b></h2></div><em className={current === "completed" ? styles.completeBadge : styles.openBadge}>{current}</em><small>{answered} / {questions.length} answered</small></header>
          {loading && <p className={styles.state}>Loading live workspace data…</p>}
          {!loading && resolution.requiredQuickContextQuestions.map((question) => <section key={question.key} className={styles.quickContext}><div><h3>Quick context</h3><p>These answers adapt conditional questions only. They are not assessed or counted.</p></div><fieldset><legend>{question.question[locale]}</legend><div>{(["yes", "no", "not_sure"] as Decision[]).map((value) => <button key={value} disabled={Boolean(saving)} onClick={() => void saveContext(question.key, value)}>{value === "not_sure" ? "Not sure" : value}</button>)}</div></fieldset></section>)}
          {questions.map((question, index) => {
            ...
            return <article id={question.id} className={styles.question} key={question.id}>
              <span className={styles.category}>{typeLabel(question.type)}</span><h3>{index + 1}. <b>{question.question[locale]}</b></h3>
              <div className={styles.answers}>{values.map((value) => <button key={value} disabled={Boolean(saving) || !workspaceId} onClick={() => { if (value === "not_applicable") { setSelectedAnswers((currentAnswers) => ({ ...currentAnswers, [question.id]: value })); return; } setSelectedAnswers((currentAnswers) => ({ ...currentAnswers, [question.id]: value })); void answer(question.id, value); }} className={`${styles[`answer_${value}`]} ${saved?.answer === value ? styles.answerSelected : ""}`}><i />{text(value, locale)}</button>)}</div>
              {(selectedAnswers[question.id] === "not_applicable" || saved?.answer === "not_applicable") && <div className={styles.justification}><label htmlFor={`justification-${question.id}`}>{locale === "fr" ? "Justification requise pour « Non applicable »" : "Required justification for Not applicable"}</label><textarea id={`justification-${question.id}`} value={justifications[question.id] ?? saved?.justification ?? ""} onChange={(event) => setJustifications((currentValues) => ({ ...currentValues, [question.id]: event.target.value }))} rows={3} disabled={Boolean(saving)} /><button type="button" disabled={!justifications[question.id]?.trim() || Boolean(saving)} onClick={() => void answer(question.id, "not_applicable")}>{locale === "fr" ? "Enregistrer la justification" : "Save justification"}</button></div>}
    ...
    <footer className={styles.footer}><button disabled={!position} onClick={() => handleNav(technologicalControls[position - 1].id)}>Previous</button><Link prefetch={true} href="/dashboard">Save & exit</Link><button disabled={position === 33} onClick={() => handleNav(technologicalControls[position + 1].id)}>Next</button></footer>
  </main>;
}
