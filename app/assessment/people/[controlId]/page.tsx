...
export default function PeopleControl() {
  ...
  async function answer(questionId: string, value: AssessmentAnswerValue) {
    if (!workspaceId || !responsesAvailable || savingId) return;
  ...
  return <main className={styles.shell}>
    ...
    <section className={styles.content}>
      <header className={styles.topbar}><p>{organization && <><span>{organization}</span><b>/</b></>}<span>{locale === "fr" ? "Évaluation" : "Assessment"}</span><b>/</b><strong>People Controls</strong></p><span className={styles.language}>EN / FR</span></header>
      <section className={styles.titleCard}><div><h1>People Controls</h1><p>ISO/IEC 27001:2022 — Annex A.6</p></div><div className={styles.titleProgress}><span>{selectedControl[1]} {locale === "fr" ? `sur ${controls.length}` : `of ${controls.length}`}</span><i><b style={{ width: `${(fullyAssessedControls / controls.length) * 100}%` }} /></i></div></section>
      <div className={styles.assessmentGrid}>
        <aside className={styles.controls}><div className={styles.controlsHeader}><div className={styles.progressRing} style={{ "--progress": `${Math.round((controls.filter(([id]) => statusFor(id) === "completed").length / controls.length) * 100)}%` } as React.CSSProperties}><span>{Math.round((controls.filter(([id]) => statusFor(id) === "completed").length / controls.length) * 100)}%</span></div><div><strong>People Controls</strong><small>{locale === "fr" ? `${controls.length} contrôles` : `${controls.length} controls`}</small></div></div>{controls.map(([id, code, en, fr]) => { const status = statusFor(id); return <a key={id} href={`/assessment/${window.location.pathname.split("/")[2]}/${id}`} onClick={(e) => { e.preventDefault(); handleNav(id); }} className={id === controlId ? styles.controlActive : ""}><i className={`${styles.status} ${styles[status]}`} /><span><strong>{code} {locale === "fr" ? fr : en}</strong><small>{status === "completed" ? (locale === "fr" ? "Terminé" : "Completed") : status === "review" ? (locale === "fr" ? "À revoir" : "Review / gap") : (locale === "fr" ? "Non commencé" : "Not started")}</small></span></a>})}</aside>
        <section className={styles.mainPanel}><header className={styles.controlHeading}><div><span>{locale === "fr" ? "Contrôle People" : "People control"}</span><h2>{selectedControl[1]} <b>{locale === "fr" ? selectedControl[3] : selectedControl[2]}</b></h2></div><em className={answerCount === questions.length && questions.length ? styles.completeBadge : styles.openBadge}>{answerCount === questions.length && questions.length ? (locale === "fr" ? "Terminé" : "Completed") : (locale === "fr" ? "Non répondu" : "Not answered")}</em><small>{answerCount} / {questions.length} {locale === "fr" ? "répondues" : "answered"}</small></header>
          ...
          {loading ? <p className={styles.state}>{locale === "fr" ? "Chargement des données réelles…" : "Loading live workspace data…"}</p> : !workspaceId ? <p className={styles.state}>{error || (locale === "fr" ? "Aucun espace d'évaluation n'est disponible." : "No assessment workspace is available.")}</p> : <>{!responsesAvailable && <p className={styles.error}>{locale === "fr" ? "Les réponses existantes ne peuvent pas encore être chargées." : "Assessment responses could not be loaded."}</p>}{questions.map((question, visibleIndex) => { const saved = responses.find((row) => row.questionId === question.id); const showJustification = notApplicableQuestionId === question.id || saved?.answer === "not_applicable"; const justification = justifications[question.id] ?? saved?.justification ?? ""; return <article id={question.id} className={styles.question} key={question.id}><span className={styles.category}>{getQuestionTypeLabel(question.type)}</span><h3>{visibleIndex + 1}. <b>{question.question}</b></h3><div className={styles.answers}>{question.responseOptions.map((value) => <button key={value} disabled={!responsesAvailable || savingId === question.id} onClick={() => void answer(question.id, value)} className={`${styles[`answer_${value}`]} ${saved?.answer === value ? styles.answerSelected : ""}`}><i />{label(value, locale)}</button>)}</div>{showJustification && <div className={styles.justification}><label htmlFor={`justification-${question.id}`}>{locale === "fr" ? "Justification requise pour « Non applicable »" : "Required justification for Not applicable"}</label><textarea id={`justification-${question.id}`} value={justification} onChange={(event) => setJustifications((current) => ({ ...current, [question.id]: event.target.value }))} rows={3} disabled={!responsesAvailable || savingId === question.id} /><button type="button" disabled={!justification.trim() || !responsesAvailable || savingId === question.id} onClick={() => void answer(question.id, "not_applicable")}>{locale === "fr" ? "Enregistrer la justification" : "Save justification"}</button></div>}<QuestionEvidence workspaceId={workspaceId} themeId="people" controlId={controlId} questionId={question.id} locale={locale} guidance={question.helpText} /></article>; })}</>}
